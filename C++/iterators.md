...
| `input_iterator`           | `++`, `*`, `See_IMPORTANT>>`                      | The `input_iterator` concept is a refinement of `input_or_output_iterator`, adding the requirement that the referenced values can be read (via `indirectly_readable`) and the requirement that the iterator concept tag be present. ***Clarification:*** *`input_or_output_iterator` doesn't require `operator*` to be readable (think `std::back_inserter`), where as `input_iterator` requires readable.* <br>**IMPORTANT:** Although input_iterator is not required to be comparable via `operator==`, it can be compared to a sentinel value.  As such, when taking them as input parameters, have `begin` be `std::input_iterator InputIterator` and `end` be `std::sentinel_for<InputIterator> Sentinel` - ranges require this.  Remember that you can use the concept in place of `typename`. |
